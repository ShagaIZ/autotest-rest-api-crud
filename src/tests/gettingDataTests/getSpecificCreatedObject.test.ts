...
test.describe('Geting data of specific created object', async () => {
	test('Valid url, with token -> get specific object data', async () => {
		const requestContext = await request.newContext()
		let response: APIResponse = await requestContext.get(`${urls.main}user/${uuidUserTwo}`, {
			headers: {
				Authorization: `Bearer ${process.env.API_KEY}`,
			},
		})
	...
	test('Invalid method -> 405 error', async () => {
		const requestContext = await request.newContext()
		let response: APIResponse = await requestContext.post(`${urls.main}user/${uuidUserTwo}`, {
			headers: {
				Authorization: `Bearer ${process.env.API_KEY}`,
			},
		})
